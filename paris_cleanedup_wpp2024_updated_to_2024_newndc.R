...
setwd("~/Documents/UW Courses/Research/CO2_Data_wpp2024_updated_to_2024_new_ndc")
...
paris.objective <- read.csv(paste0(data.location, 'paris_agreement_new_ndc.csv'))# change
...
for (i in 1:54)
{
  ...
  fit_result <- fit_ar1_model(all_hist_df, model_x, n.iterations = 10000) # original = 10000
  
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.ar1.const$trajs.annual.worldwide, rcp.carbon.cum.complete, 
                                   model_x, year.range = c(2024, 2100), n.traj = 1000) 
...
for (i in 1:54)
{
  ...
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.adjusted$trajs.annual.worldwide.cont, 
                                         rcp.carbon.cum.complete, model_x, year.range = c(2024, 2100), n.traj = 1000)
  all_fits[[model_x]] <- fit_result
  all_projections[[model_x]] <- proj_list_model
...
{
  model_x <- all_models[i]
  fit_result <- fit_ar1_model(all_hist_df, model_x, n.iterations = 10000) # fit_result
