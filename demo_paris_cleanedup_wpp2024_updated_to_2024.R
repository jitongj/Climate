...
load(file=paste0(data.location, "model_results_ar1const_2024.rda"))
load(file=paste0(data.location, "proj_evals_ar1const_2024.rda"))
load(file=paste0(data.location, "data_medium_wpp2024_updated2024.Rda"))
...
# load("~/Documents/Research/Climate/Data/proj.evals.2024.adjusted.new.Rda")
real_data = hadcrut5
...
convert.traj <- function(proj.evals, paris.objective, policy.continue=FALSE)
{
  ...
  corr_level <- as.numeric(model.ar1.const.results.2024$corr.model.out[[2]][[1]][param.indeces, 'rho'])
  for (j in 2:5)
  {
    corr_level <- c(corr_level, as.numeric(model.ar1.const.results.2024$corr.model.out[[2]][[j]][param.indeces, 'rho']))
  }
  ...
  for (l in 1:nrow(paris.objective))
  {
    ...
    if (type == 'Emission' || (type == 'BAU' && !is.na(paris.objective$ratio[l])))
    {
      ...
      year_ind <- ceiling((paris.objective$year[l] - 2023) / 1) # change to 2023 since start year 2024
      if (policy.continue) {exponential_change <- (0:76) / (year_ind - 1) } # change to 76 means starting at 2024 (2024+76 = 2100)
      else {exponential_change <- c((0:(year_ind - 1))/(year_ind - 1), rep(1, 77-year_ind))} # change to 77 means starting at 2024 (2023+77=2100)
...
compute.ref.level <- function(proj.evals, paris.objective, policy.continue=FALSE)
{
  ...
  for (l in 1:nrow(paris.objective))
  {
    ...
    else if (type == 'Intensity'|| (type == 'BAU' && is.na(paris.objective$old_year[l])))
    {
      ...
      ref_level <- data.medium$Tech[(data.medium$Isocode == country_code) & (data.medium$Year == 2024)] * (1 - size / 100)
      type <- 'Intensity'
      ref_levels <- c(ref_levels, ref_level)
      types <- c(types, type)
    }
...
# not useful, can be ingored
#res <- compute.ref.level(proj.evals.2024.ar1.const, paris.objective)
...
convert.traj.annual <- function(proj.evals, paris.objective, policy.continue=FALSE)
{
  ...
  for (l in 1:nrow(paris.objective))
  {
    ...
    if (type == 'Emission' || (type == 'BAU' && !is.na(paris.objective$ratio[l])))
    {
      old_year <- paris.objective$old_year[l]
      size <- ifelse(type == 'Emission', paris.objective$size[l], size <- paris.objective$ratio[l])
      year_ind <- paris.objective$year[l] - 2023 # change to 2023 since start year 2024
      if (policy.continue) {exponential_change <- (0:76) / (year_ind - 1)} # change to 76 means starting at 2024
      else {exponential_change <- c((0:(year_ind - 1))/(year_ind - 1), rep(1, 77-year_ind))} # change to 77 means starting at 2024(2023)
    ...
    else if (type == 'Intensity'|| (type == 'BAU' && !is.na(paris.objective$old_year[l])))
    {
      ...
      year_ind <- paris.objective$year[l] - 2023 # change to 2023 since start year 2024
      if (policy.continue) {exponential_change <- (0:76) / (year_ind - 1)} # change to 76 means starting at 2024
      else {exponential_change <- c((0:(year_ind - 1))/(year_ind - 1), rep(1, 77-year_ind))} # change to 77 means starting at 2024
      ref_level <- data.medium$Tech[(data.medium$Isocode == country_code) & (data.medium$Year == old_year)] * (1 - size / 100)
      for (j in 1:length(ipat.components.bycountry))
      {
        if (ipat.components.bycountry[[j]][[country_code]]$Tech[year_ind] < ref_level)
...
calculate.prob <- function(proj.evals, paris.objective)
{
  ipat.components.bycountry <- proj.evals$ipat.annual.components.bycountry
  param.indeces <- round(seq(1, floor(1e5 / 20), length.out=(length(ipat.components.bycountry) / 5)))
  countries <- names(proj.evals.2024.ar1.const$ipat.annual.components.bycountry[[1]])
...
converted_trajs <- convert.traj(proj.evals.2024.ar1.const, paris.objective)
converted_annual_trajs <- convert.traj.annual(proj.evals.2024.ar1.const, paris.objective)
converted_annual_trajs_cont <- convert.traj.annual(proj.evals.2024.ar1.const, paris.objective, TRUE)

proj.evals.2024.adjusted <- list(ipat.components.bycountry = converted_trajs$ipat.components.bycountry, 
...
proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont <- 
...
trajs.worldwide <- lapply(1:length(proj.evals.2024.adjusted$ipat.components.bycountry), function(i) {
  x <- proj.evals.2024.adjusted$ipat.components.bycountry[[i]]
  get.ipat.components.total(x, year.sequence=seq(year.start, year.end,1))
})

trajs.annual.worldwide <- lapply(1:length(proj.evals.2024.adjusted$ipat.annual.components.bycountry), function(i) {
  x <- proj.evals.2024.adjusted$ipat.annual.components.bycountry[[i]]
  get.ipat.components.total(x, year.sequence=year.start:year.end)
})

trajs.annual.worldwide.cont <- lapply(1:length(proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont), function(i) {
  x <- proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont[[i]]
  get.ipat.components.total(x, year.sequence=year.start:year.end)
})

trajs.annual.worldwide.usa <- lapply(1:length(proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont), function(i) {
  x <- proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont[[i]]
  x$USA <- proj.evals.2024.ar1.const$ipat.annual.components.bycountry[[i]]$USA
  get.ipat.components.total(x, year.sequence=year.start:year.end)
})

proj.evals.2024.adjusted$trajs.worldwide <- trajs.worldwide
proj.evals.2024.adjusted$trajs.annual.worldwide <- trajs.annual.worldwide
proj.evals.2024.adjusted$trajs.annual.worldwide.cont <- trajs.annual.worldwide.cont
proj.evals.2024.adjusted$trajs.annual.worldwide.usa <- trajs.annual.worldwide.usa

proj.evals.2024.adjusted$probability <- calculate.prob(proj.evals.2024.ar1.const, paris.objective)
## World Quantiles:
...
proj.evals.2024.adjusted$trajs.quants <- get.trajs.quants(proj.evals.2024.adjusted$trajs.worldwide, 
                                                          quantiles = c(0.025, 0.05, 0.5, 0.95, 0.975))
proj.evals.2024.adjusted$trajs.annual.quants <- 
  get.trajs.quants(proj.evals.2024.adjusted$trajs.annual.worldwide, 
                   quantiles = c(0.025, 0.05, 0.5, 0.95, 0.975))
proj.evals.2024.adjusted$trajs.annual.quants.cont <- 
  get.trajs.quants(proj.evals.2024.adjusted$trajs.annual.worldwide.cont, 
                   quantiles = c(0.025, 0.05, 0.5, 0.95, 0.975))
proj.evals.2024.adjusted$trajs.annual.quants.usa <- 
  get.trajs.quants(proj.evals.2024.adjusted$trajs.annual.worldwide.usa, 
                   quantiles = c(0.025, 0.05, 0.5, 0.95, 0.975))
...
rcp.carbon.yearly <- rcp.carbon.yearly.tmp[, c("Scenario", "Unit",
                                               c(2000, 2005, seq(2010, 2100, by=10)))]
...
for (i in 1:4) {
  cumulative_values <- cumsum(as.numeric(rcp.carbon.yearly.complete[i, 4:98]))
  rcp.carbon.cum.complete[i, 4:98] <- matrix(cumulative_values, nrow = 1)
}
...
all_models <- unique(all_models_split[seq(2, 5282, 4)])
selected_indices <- c()
...
# project_xt <- function(carbon_trajectory, rcp_co2, model_x, year.range = c(2005, 2110), start.from.2024=TRUE)
# {
#   co2.trajectory <- carbon_trajectory$CO2 / 1e9
#   carbon.cum <- 0
...
project_xt <- function(carbon_trajectory, year.range = c(2005, 2110), start.from.2024=TRUE)
{
  co2.trajectory <- carbon_trajectory$CO2 / 1e9
  carbon.cum <- 0
  proj_length <- length(year.range[1]:year.range[2]) - 1
  ...
  y1 <- real_data_adjusted$V6[nrow(real_data_adjusted)] # temp at 2024
  x1 <- co2.trajectory[1] # cum co2 at 2024
  slope <- rnorm(1, mean = 0.45, sd = 0.18)/1000 # AR6 distr
...
project_temperature <- function(fit_result_chain, carbon.projection.trajectories, rcp_co2, model_x, n.traj = 20, 
                                year.range = c(2005, 2110), ratio_co2 = 1, start.from.2024=TRUE)
{
  num_carbon_trajectories <- length(carbon.projection.trajectories)
  ...
  {
    proj_xt <- project_xt(carbon.projection.trajectories[[carbon_trajs[i]]] * ratio_co2,
                          year.range, start.from.2024=start.from.2024)
    proj_zt <- project_zt(fit_result_chain, zt_trajs[i], year.end = year.range[2], year.start = year.range[1], proj_xt=proj_xt)
    proj_xt <- proj_xt[((year.range[1]+1):year.range[2])-2024] # change 2005 to 2024
    # proj_zt_cutting <- proj_zt[5 * (1:length_proj)]
    # proj_zt_cutting <- proj_zt[seq(year.range[1], year.range[2], 5) - 2005]
...
for (i in 1:54)
{
  ...
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.ar1.const$trajs.annual.worldwide, rcp.carbon.cum.complete, 
                                   model_x, year.range = c(2024, 2100), n.traj = 20) 
...
for (i in 1:54)
{
  ...
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.adjusted$trajs.annual.worldwide, rcp.carbon.cum.complete, 
                                         model_x, year.range = c(2024, 2100), n.traj = 20)
  all_fits[[model_x]] <- fit_result
  all_projections[[model_x]] <- proj_list_model
...
for (i in 1:54)
{
  ...
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.adjusted$trajs.annual.worldwide.cont, 
                                         rcp.carbon.cum.complete, model_x, year.range = c(2024, 2100), n.traj = 20)
  all_fits[[model_x]] <- fit_result
  all_projections[[model_x]] <- proj_list_model
...
for (i in 1:54)
{
  ...
  proj_list_model <- project_temperature(fit_result, proj.evals.2024.adjusted$trajs.annual.worldwide.usa, 
                                         rcp.carbon.cum.complete, model_x, year.range = c(2024, 2100), n.traj = 20)
  all_fits[[model_x]] <- fit_result
  all_projections[[model_x]] <- proj_list_model
...
probability <- data.frame(iso = proj.evals.2024.adjusted$probability$country, un = NA,
                          prob = proj.evals.2024.adjusted$probability$probability)
...
true_anomaly_names <- paste('true_anomaly[', 1:145, ']', sep = '') # 1:145: 1861 to 2005
...
plot_temp <- function(summary_proj, summary_anomaly, title = 'Anomaly Forecast')
{
  ...
  other_estimates <- data.frame(year = 2006:2024, observed_anomaly = real_data$V6[157:175] - mean(real_data$V6[12:31])) # change to 157:175(start=2006,end=2024)
  other_quantiles <- other_estimates$observed_anomaly + 
    matrix(rep(qnorm(c(0.025, 0.05, 0.1, 0.5, 0.9, 0.95, 0.975)), each = 19), nrow= 19)# change to 19 because 2006 to 2024
                             ...
                             title, ipat.quantiles.bycountry, yaxis = TRUE)
{
  ...
  if(intensity)
  {
    range_min <- min(data.medium$Tech[data.medium$Isocode == Iso], 
                     proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry[[Iso]]$`0.025`$Tech, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry[[Iso]]$`0.025`$Tech, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont[[Iso]]$`0.025`$Tech)
    range_max <- max(data.medium$Tech[data.medium$Isocode == Iso], 
                     proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry[[Iso]]$`0.975`$Tech, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry[[Iso]]$`0.975`$Tech, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont[[Iso]]$`0.975`$Tech)
  ...
  else
  {
    ...
    range_min <- min(data.medium$CO2.total[data.medium$Isocode == Iso] * 11 / 3e3, 
                     proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry[[Iso]]$`0.025`$CO2 / 1e9, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry[[Iso]]$`0.025`$CO2 / 1e9, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont[[Iso]]$`0.025`$CO2 / 1e9)
    range_max <- max(data.medium$CO2.total[data.medium$Isocode == Iso] * 11 / 3e3, 
                     proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry[[Iso]]$`0.975`$CO2 / 1e9, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry[[Iso]]$`0.975`$CO2 / 1e9, 
                     proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont[[Iso]]$`0.975`$CO2 / 1e9)
    
    data.historical$CO2Total <- data.historical$CO2 * (1000 * data.historical$Pop)/1e9
    # data.2024 <- data.frame(Year = 2024, CO2 = newdata)
    for(quant in quantiles.need)
    {
    ...
    plot.obj <- plot.obj + geom_line(data = data.historical, aes(x=Year, y = CO2Total), size = 1.2) + 
      geom_point(aes(x = Year, y = obj), data = obj.data, color="Blue", size = 4) # + 
    # geom_point(aes(x = Year, y = CO2), data = data.2024,  color="black", size = 4)
  }
  ...
  for (iso in names(ipat.components.bycountry[[1]])) {
    ...
    cumsum_co2 <- sapply(1:length(ipat.components.bycountry), function(i)
      ...
      rbind(0, sapply(1:length(ipat.components.bycountry), function(i)
        cumsum(ipat.components.bycountry[[i]][[iso]][-77, var.name]))) / 2 # change to 77 (2024 to 2100)
...
get.ipat.cum.quantiles <- function(trajs.worldwide, 
  ...
  cumsum_co2 <- sapply(1:length(trajs.worldwide), function(i)
    ...
    rbind(0, sapply(1:length(trajs.worldwide), function(i)
      cumsum(trajs.worldwide[[i]][-77, var.name]))) / 2 # change to 77 (2024 to 2100)
...
ipat.quantiles.bycountry <- get.ipat.quantiles.bycountry(proj.evals.2024.adjusted$ipat.components.bycountry)
ipat.annual.quantiles.bycountry <- 
  get.ipat.quantiles.bycountry(proj.evals.2024.adjusted$ipat.annual.components.bycountry)
ipat.annual.quantiles.bycountry.cont <- 
  get.ipat.quantiles.bycountry(proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont)

proj.evals.2024.adjusted$ipat.quantiles.bycountry <- ipat.quantiles.bycountry
proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry <- ipat.annual.quantiles.bycountry
proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont <- ipat.annual.quantiles.bycountry.cont

proj.evals.2024.ar1.const$ipat.quantiles.cum.bycountry <- 
  get.ipat.cum.quantiles.bycountry(proj.evals.2024.ar1.const$ipat.annual.components.bycountry)
proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry <- 
  get.ipat.cum.quantiles.bycountry(proj.evals.2024.adjusted$ipat.annual.components.bycountry)
proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry.cont <- 
  get.ipat.cum.quantiles.bycountry(proj.evals.2024.adjusted$ipat.annual.components.bycountry.cont)

save(proj.evals.2024.adjusted, file = 'proj.evals.2024.adjusted.new.Rda')
font <- 'Times'
...
# plot.co2.projections(data.medium, proj.evals.2024.adjusted$trajs.annual.quants)
plot.co2.projections(data.medium, proj.evals.2024.ar1.const$trajs.annual.quants) # change
proj.evals.2024.adjusted$ipat.quantiles.bycountry <- ipat.quantiles.bycountry
proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry <- ipat.annual.quantiles.bycountry
proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont <- ipat.annual.quantiles.bycountry.cont
...
plot.location <- 'Paris/'
font <- 'Times'
for (country_name in proj.evals.2024.adjusted$adjusted_countries)
{
  ...
  if (paris.obj.country$Type == 'Intensity')
  {
    ...
    p <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = T,  # change to 77
                          title = paste0("Adjusted"), 
                          ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry, yaxis = F)
    p1 <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = T, # change to 77
                           title = paste0("Continued"),
                           ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont, yaxis = F)
    p2 <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = T,# change to 77
                           title = paste0("None"),
                           ipat.quantiles.bycountry = proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry)
  }
  else if (paris.obj.country$Type == 'Emission')
  {
    ...
    p <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = F, # change
                          title = paste0('Adjusted'),
                          ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry, yaxis = F)
    p1 <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = F, # change
                           title = paste0("Continued"),
                           ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont, yaxis = F)
    p2 <- plot.paristarget(country_name, obj, indeces = 1:77, intensity = F, # change
                           title = paste0("None"),
                           ipat.quantiles.bycountry = proj.evals.2024.ar1.const$ipat.annual.quantiles.bycountry)
  }
  else
...
plot.cum.by.country <- function(Iso, indeces = 1:77, title, ipat.quantiles.bycountry) # change
{
  color.us <- "#e41a1c"
  plot.obj <- ggplot() +
    labs(color="Scenario") +
  ...
  quantiles.need <- c(0.025, 0.05, 0.5, 0.95, 0.975)
  range_min <- min(proj.evals.2024.ar1.const$ipat.quantiles.cum.bycountry[[Iso]]$`0.025` / 1e9, 
                   proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry[[Iso]]$`0.025` / 1e9, 
                   proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry.cont[[Iso]]$`0.025` / 1e9)
  range_max <- max(proj.evals.2024.ar1.const$ipat.quantiles.cum.bycountry[[Iso]]$`0.975` / 1e9, 
                   proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry[[Iso]]$`0.975` / 1e9, 
                   proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry.cont[[Iso]]$`0.975` / 1e9)
...
for (country_name in proj.evals.2024.adjusted$adjusted_countries)
{
  ...
  p <- plot.cum.by.country(country_name, 
                           ...
                           ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry)
  p1 <- plot.cum.by.country(country_name, 
                            title = paste0(paris.objective$Country[paris.objective$country_code == country_name], "'s Emission"), 
                           ipat.quantiles.bycountry = proj.evals.2024.adjusted$ipat.quantiles.cum.bycountry.cont)
  p2 <- plot.cum.by.country(country_name, 
                            title = paste0(paris.objective$Country[paris.objective$country_code == country_name], "'s Emission"), 
                           ipat.quantiles.bycountry = proj.evals.2024.ar1.const$ipat.quantiles.cum.bycountry)
...
proj.evals.2024.adjusted$trajs.cum.quants <- get.ipat.cum.quantiles(proj.evals.2024.adjusted$trajs.annual.worldwide)
proj.evals.2024.adjusted$trajs.cum.quants.cont <- get.ipat.cum.quantiles(proj.evals.2024.adjusted$trajs.annual.worldwide.cont)
proj.evals.2024.ar1.const$trajs.cum.quants <- get.ipat.cum.quantiles(proj.evals.2024.ar1.const$trajs.annual.worldwide)
...
p1 <- plot.cum.emission(proj.evals.2024.ar1.const$trajs.cum.quants, 'Cumulative Emission')
p2 <- plot.cum.emission(proj.evals.2024.adjusted$trajs.cum.quants, 'Cumulative Emission with Paris Agreement')
p3 <- plot.cum.emission(proj.evals.2024.adjusted$trajs.cum.quants.cont, 'Cumulative Emission with Policy Continued')


save(proj.evals.2024.adjusted, file = 'proj_evals_2024_adjusted.Rda')
...
#     proj_list <- project_temperature(fit_result, proj.evals.2024.adjusted$trajs.annual.worldwide.cont, rcp.carbon.cum.complete, rcp_temp_data, 
#                                      year.range = c(2024, 2100), n.traj = 1000, ratio_co2 = ratio_co2)
#     proj_temp <- proj_list$projection
#     summary_proj <- data.frame(year = proj_temp[,1])
#     # browser()
...
# 
# 
# cumulative_co2 <- sapply(1:1000, function(i) {return (sum(proj.evals.2024.adjusted$trajs.annual.worldwide.cont[[i]]$CO2/1e9))})
...
plot.intensity <- function(Iso, version = 2010)
{
  ...
  range_min <- min(proj.evals.2010.ar1.const$ipat.quantiles.bycountry[[Iso]]$`0.05`$Tech, 
                   proj.evals.2024.ar1.const$ipat.quantiles.bycountry[[Iso]]$`0.05`$Tech, 
  ...
  range_max <- max(proj.evals.2010.ar1.const$ipat.quantiles.bycountry[[Iso]]$`0.95`$Tech, 
                   proj.evals.2024.ar1.const$ipat.quantiles.bycountry[[Iso]]$`0.95`$Tech, 
                   data.medium$Tech[data.medium$Isocode == Iso],
                   data.medium.new$Tech[data.medium.new$Isocode == Iso])
  ...
  {
    data.plot <- data.frame(Year = seq(2024, 2100, 1))
  }
  ...
  for(quant in quantiles.need)
  {
    ...
    {
      data.plot[[paste0('quantiles', quant)]] <- 
        proj.evals.2024.ar1.const$ipat.quantiles.bycountry[[Iso]][[as.character(quant)]]$Tech
    }
  }
  plot.obj <- plot.obj + 
    ylab('Intensity') + 
...
#   p1 <- plot.intensity(Iso, version = 2024)
#   
...
ratio_emission <- data.frame(Iso = names(proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry))
...
total <- proj.evals.2024.adjusted$trajs.annual.quants.cont$CO22100[3]
for (i in 1:nrow(ratio_emission))
{
  iso <- as.character(ratio_emission$Iso[i])
  proportion <- rev(proj.evals.2024.adjusted$ipat.annual.quantiles.bycountry.cont[[iso]]$`0.5`$CO2)[1]/1e9
...
for (i in 1:54)
{
  ...
  proj_list <- project_temperature(fit_result, proj.evals.2024.ar1.const$trajs.annual.worldwide, rcp.carbon.cum.complete, 
                                   model_x, year.range = c(2024, 2100), n.traj = 20)
  all_fits[[model_x]] <- fit_result
...
combined.results$Year <- 2025:2100 # change
...
plot_temp_new <- function(summary_proj, summary_anomaly, title = 'Anomaly Forecast')
{
  ...
  other_quantiles <- other_estimates$observed_anomaly + 
    matrix(rep(qnorm(c(0.025, 0.05, 0.1, 0.5, 0.9, 0.95, 0.975)), each = 19), nrow=19) # change to 19 since start from 2024
  other_quantiles <- data.frame(other_quantiles)
  names(other_quantiles) <- c("q025", "q050", "q100", "median", "q900", "q950", "q975")
  other_estimates <- cbind(other_estimates, other_quantiles)
  summary_anomaly <- rbind(summary_anomaly, other_estimates)
...
plot_temp_none <- function(summary_proj, summary_anomaly, title = 'Assembled Global Mean Temperature Forecast')
{
  ...
  other_estimates <- data.frame(year = 2006:2024, observed_anomaly = real_data$V6[157:175] - mean(real_data$V6[12:31])) # change to 175 due to 2024
  other_quantiles <- other_estimates$observed_anomaly + 
    matrix(rep(qnorm(c(0.025, 0.05, 0.1, 0.5, 0.9, 0.95, 0.975)), each = 19), nrow=19) # change to 19 since start year is 2024
  other_quantiles <- data.frame(other_quantiles)
  names(other_quantiles) <- c("q025", "q050", "q100", "median", "q900", "q950", "q975")
  other_estimates <- cbind(other_estimates, other_quantiles)
  summary_anomaly <- rbind(summary_anomaly, other_estimates)
  ...
  p1 <- ggplot(data = summary_proj_none, aes(x = year))  + 
    ...
    geom_hline(yintercept = seq(-0.5,4.5,0.5), color = 'grey') +
    geom_vline(xintercept = c(1900, 2000, 2050, 2100), color = 'grey') + 
    geom_vline(xintercept = 2025, linetype = "dashed", color = "red", size = 1) +  # 添加2025年的竖线
    annotate("text", x = 2025, y = max(summary_proj_none$median), label = "2025", 
             color = "red", vjust = -1.5, size = 5) +  # 在竖线附近添加“2025”的标签
...
for (country in c('USA', 'JPN', 'DEU', 'RUS', 'CAN', 'KOR', 'BRA', 'GBR'))
{
  ...
  rate <- 1 - rate^(1/(paris.objective$year[l] - 2024))
  for(j in 1:3)
  {
    obj <- data.medium$CO2.total[(data.medium$Isocode == country) & 
                                   (data.medium$Year == 2024)] * (1 - rate * rates[j+1]/rates[1])^(paris.objective$year[l] - 2024)
